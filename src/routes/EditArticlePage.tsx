...
export const EditArticlePage = () => {
  ...
  useEffect(() => {
    for (const index in tagList) {
      ...
      // @ts-expect-error
      setTags((prev) => [...prev, { id: tagId, tag: <Tag tagId={tagId} handleDeleteTag={handleDeleteTag} setTagsText={setTagsText} text={tagList[index]} /> }])
  ...
  const updateAnArticle = async (articleData: articleData) => {
  ...
  const handleSubmitFull =
    handleSubmit(async (dataForm) => {
      ...
      try {
        const data = await updateAnArticle(updateArticle as articleData);
        console.log(data)
        if (data?.article) {
  ...
  const handleAddTag = (event: Event) => {
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    setTags([...tags, { id: tagId, tag: <Tag tagId={tagId} handleDeleteTag={handleDeleteTag} setTagsText={setTagsText} /> }])
  };
